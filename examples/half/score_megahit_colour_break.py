#!/usr/bin/env python3
"""Apply read-colour breaks to MEGAHIT final contigs and score them."""
...
def main() -> int:
    ...
    payload: dict = {}
    dest = out_dir / "megahit_colour_break_metrics.json"
    if dest.is_file():
    ...
    for name in names:
        ...
        for path in (final, *reads, ref):
            if not path.is_file():
                raise SystemExit(f"missing {path}")
        print("breaking", name, flush=True)
        broken = break_read_colour_chimeras(read_fasta(final), reads, k=21, min_run=40, min_piece=80)
        fasta = out_dir / f"{name}_megahit_colour_break.fasta"
        ...
        payload[name] = {"megahit_final": baseline, "read_colour_break": mode, "wins": wins}
